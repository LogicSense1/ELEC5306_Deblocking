...
def generate_single_batch(train_input_dir, train_label_dir, batch_paths, crop_flag):
    ...
    for i in range(len(batch_paths)):
        train_label_image_path = os.path.join(train_label_dir, batch_paths[i])
        train_input_image_path = os.path.join(train_input_dir, batch_paths[i][:-5] + '_q40_' + batch_paths[i][-5:-4] +'.jpg')
        if i == len(batch_paths)-1:
            train_input_image_path2 = os.path.join(train_input_dir, batch_paths[i-1][:-5] + '_q40_' + batch_paths[i-1][-5:-4] +'.jpg')
        else:
            train_input_image_path2 = os.path.join(train_input_dir, batch_paths[i+1][:-5] + '_q40_' + batch_paths[i+1][-5:-4] +'.jpg')
        img_label = Image.open(train_label_image_path)
        img_input = Image.open(train_input_image_path)
        img_input2 = Image.open(train_input_image_path2)
        ...
        if crop_flag:
            ...
            img_input2 = img_input2.crop((width / 4, height / 4, 3 * width / 4, 3 * height / 4))
        train_label_patches.append(np.asarray(img_label))
        img_input = np.dstack((np.asarray(img_input), np.asarray(img_input2)))
