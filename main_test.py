...
parser.add_argument('--model_choice', default='memnet', type=str, help='choose the model type')
parser.add_argument('--QT_FACTOR', default=40, type=int, help='quantization factor')
parser.add_argument('--test_batchsize', default=7, type=int, help='training batches')
parser.add_argument('--InputFolder', default='/home/wentian/Documents/ELEC5306_Deblock/trainset/vimeo_part_q40_crop', type=str, help='inputfolder')
parser.add_argument('--LabelFolder', default='/home/wentian/Documents/ELEC5306_Deblock/trainset/vimeo_part_crop', type=str, help='LabelFolder')
parser.add_argument('--model_dir', default='/home/wentian/Documents/ELEC5306_Deblock/models/memnet_q40/checkpoint_q40_250.pth.tar', type=str, help='model path')
parser.add_argument('--Testlist', default='/home/wentian/Documents/ELEC5306_Deblock/lists/temp_sep_trainlist.txt', type=str,
                    help='testlist')  # 7824
        ...
        with torch.no_grad():
            for n_count, batch_yx in enumerate(DLoader_test):
                ...
                for s in range(0, model_output.shape[0]):
                    psnrs = calculate_psnr(batch_y[s][0:3,:,:], model_output[s], batch_x[s])
                    psnr_pre = psnrs[0]
                    psnr_input = psnrs[1]
                    ...
                    if iteration == 0:
                        pred = model_output[s].mul_(255.0).clamp_(0.0, 255.0).squeeze(0).permute(1, 2, 0).byte().cpu().numpy()
                        output = Image.fromarray(pred, mode='RGB')
                        output.save(os.path.join('/home/wentian/Documents/ELEC5306_Deblock/output', '{}_{}_{}.png'.format(iteration, i, s)))
